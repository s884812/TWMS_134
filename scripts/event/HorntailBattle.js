...
function setup(eim, leaderid) {
    ...
    var map = eim.getMapFactory(em.getWorld()).getMap(240060000);
    map.killAllMonsters(false);

    map = eim.getMapFactory(em.getWorld()).getMap(240060100);
    map.killAllMonsters(false);

    map = eim.getMapFactory(em.getWorld()).getMap(240060200);
    map.killAllMonsters(false);
    map.resetReactors();
...
function CheckHorntailHead(eim) {
	...
	eim.getMapFactory(em.getWorld()).getMap(240060000).spawnMonsterOnGroundBelow(mob, new java.awt.Point(890, 230));
	...
	eim.registerMonster(mob);
	eim.getMapFactory(em.getWorld()).getMap(240060100).spawnMonsterOnGroundBelow(mob, new java.awt.Point(-360, 230));
    }
}

function playerEntry(eim, player) {
    var map = eim.getMapFactory(em.getWorld()).getMap(240060200);
