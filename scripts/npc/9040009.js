	...
	if (eim.getProperty("leader").equals(cm.getName())) {
		...
		if (status == 0) {
			...
			if (eim.getProperty("stage1combo").equals(eim.getProperty("stage1guess"))) {
			    if (stage == 3) {
				cm.getMap().getReactorByName("statuegate").hitReactor(cm.getC());
		    ...
		    var delay = 5000;
		    for (var i = 0; i < combo.length; i++) {
			cm.getMap().getReactorByOid(combo[i]).delayedHitReactor(cm.getC(), delay + 3500*i);
		    }
...
function getReactors() {
    ...
    var iter = cm.getMap().getMapObjects().iterator();
    while (iter.hasNext()) {
	var mo = iter.next();
